...
class NPUCudaGraphBackend(BaseCudaGraphBackend):
    ...
    def replay_with_input_update(
        ...
        ``NPUGraph.update`` must finish before replay starts. Replaying while
        the update is still running can use the capture-time sequence lengths,
        which makes target verification reject otherwise valid draft tokens.
        Used when the model is not deepseek-nsa.
