...
def weak_ref_tensors(
    ...
    if isinstance(tensors, list):
        return [weak_ref_tensor(t) if isinstance(t, torch.Tensor) else t for t in tensors]
    if isinstance(tensors, tuple):
        return tuple(weak_ref_tensor(t) if isinstance(t, torch.Tensor) else t for t in tensors)
