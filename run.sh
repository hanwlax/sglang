...
export SGLANG_DEBUG_NPU_BATCH_TIMING="${SGLANG_DEBUG_NPU_BATCH_TIMING:-1}"
export SGLANG_LOG_FORWARD_ITERS="${SGLANG_LOG_FORWARD_ITERS:-1}"
export SGLANG_LOG_MS="${SGLANG_LOG_MS:-1}"
...
CUDA_GRAPH_BS="${CUDA_GRAPH_BS:-2 4 8 12 16}"
...
# Discard inherited code overrides before CANN restores its Python dependencies.
unset PYTHONPATH
...
export SGLANG_K3_NPU_SHARED_EXPERTS_DUAL_STREAM=0  # 使用双流：1

# MXFP8 quant
export SGLANG_NPU_MOE_SITU_MXFP8_FUSED=0

# V2 NZ
export SGLANG_NPU_USE_FIAS_V2_BSND=1
export SGLANG_NPU_USE_MLAPO=0
export SGLANG_USE_FIA_NZ=0
export SGLANG_NPU_ENABLE_EARLY_TARGET_GRAPH_UPDATE=0

# Experts dual stream
export SGLANG_NPU_FINE_GRAINED_MOE_DUAL_STREAM=True

# PYTHONPATH
CODEPATH="/home/hanwlax/test-codes/k3"
SGLANG_PATH="${CODEPATH}/sglang-nz/python"
KERNEL_PATH="${CODEPATH}/sgl-kernel-npu/python/sgl_kernel_npu"
export PYTHONPATH="${SGLANG_PATH}${KERNEL_PATH:+:${KERNEL_PATH}}${PYTHONPATH:+:${PYTHONPATH}}"
...
do
    ...
    then
	    ...
	    --enable-shared-experts-attn-tp --enable-dense-mlp-attn-tp \
...
curl --location 'http://127.0.0.1:30000/start_profile' \
  ...
  --data '{
    "output_dir": "/home/hanwlax/workspace/progress/kimi_k3/profiling/0827-alt-stream",
    "num_steps": 18,
    "profile_by_stage": false,
    "with_stack": false,
    "profile_prefix": "no-alt-stream"
  }'

curl --location 'http://127.0.0.1:30000/stop_profil'
