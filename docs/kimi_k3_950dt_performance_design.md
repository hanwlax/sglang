...
description: "基于 a5-k3-0828 分支，描述共享专家双流、FIAS V2 接入及 SiTU 与 MXFP8 量化融合的设计。"
---
...
本文面向 Ascend 950DT 上运行 Kimi K3 的场景，以缩短推理关键路径、减少算子发射和中间数据搬运为目标，描述 `a5-k3-0828` 分支中的三项性能优化：共享专家双流、FIAS V2 接入，以及 SiTU 与 MXFP8 激活量化融合。

在本文涉及的 ModelSlim W4A8 MoE 路径中，权重采用 packed MXFP4，grouped matmul 的激活输入采用 MXFP8。GMM1 产生的 BF16 gate/up 输出经过 SiTU 后，必须量化为与 GMM2 接口匹配的 FP8 payload 和块级 scale。融合算子优化的正是这段**运行时激活量化**，权重离线量化与加载时的格式转换沿用已有实现。

| 修改项 | 对应的性能问题 | 设计结果 |
| --- | --- | --- |
| 共享专家双流 | Attention-TP 共享专家路径未参与已有重叠执行；共享 MLP 与 routed experts 串行增加尾部耗时 | 共享 MLP 在备用流执行，与主流 routed 分支重叠；共享分支的 all-gather/reduce-scatter 保留在主流 |
| FIAS V2 接入 | DSpark target verify 与 draft MHA 需要接入新的 attention 执行接口，并正确处理布局和图内动态长度 | MLA verify 使用 V2 的 BNSD 输入；MHA 保持 TND；统一适配 V2 长度参数与图更新顺序 |
| SiTU＋MXFP8 Quant | SiTU 与 GMM2 前的量化分开发射，中间 BF16 数据落到 GM，量化可能扫描整块容量 | 单个 AscendC kernel 完成 SiTU、BF16 舍入及 MXFP8 量化，只处理有效专家行 |

实现范围按三个 SGLang 提交确定：双流以 `f3ca9cb559` 为准，FIAS V2 以 `8491b10adf` 为准，融合算子接入以 `ff2f4caf69` 为准；外部算子实现以 `bc34b34725` 为准。编制时核对的分支快照为 `63aa7f7ca193eb628359b651f00553853ce77f65`，核对日期为 2026-09-14。

本文仅展开三项修改及其必要的调用约定。分支后续的细粒度共享专家重叠、共享专家 TP 子组、NZ 缓存扩展及其他调度优化不另列为本文修改项。对应 commit 链接见第 8 节。

## 2. 优化位置与总体关系

Kimi K3 的单层执行中，attention 与 MoE 是不同的计算阶段；MoE 内部又包含共享专家和 routed experts 两条可并行的分支。三项优化分别作用于不同位置。
...
三项改动的收益需要分别测量：双流改变执行重叠，FIAS V2 改变 attention 调用，SiTU 融合减少 routed 分支内部的工作。融合缩短 routed 分支后，原先被隐藏的共享分支耗时可能重新暴露，因此各项加速比不能直接相乘。
...
在指定提交之前，`_sbo_shared_overlap` 的启用条件排除了 `_shared_experts_attn_tp_comm`。这使 NPU attention-TP 兼容模式即使已具备备用流，也不能使用共享专家重叠。该提交移除此排除条件，并把共享分支拆成三个执行部分：
...
FIAS V2 接入提交将更新顺序改为先完成 `graph.update()`，再调用 `graph.replay()`。编制时的分支已将 update 放入复用的设备绑定 worker，但仍等待 `update_future.result()` 后才 replay；这一后续实现保持了同一正确性约束，不属于本文新增的第四项优化。
...
| 项目 | 条件 / 默认行为 | 集成要求 |
| --- | --- | --- |
| 共享专家双流 | 指定提交按 EP、共享专家和备用流条件自动选择 | 共享输入跨流有效；AG/RS 与 routed collective 的 rank 顺序一致 |
...
串行双流对照应保持相同 EP 和共享专家分片，使用只改变 shared 分支调度的受控构建。改为无 EP 或移除共享专家会同时改变计算和通信，不能作为有效对照。FIAS V2 和 SiTU 融合的对照使用第 6 节中的两个环境开关。
...
分支集成快照：[63aa7f7ca1](https://github.com/zhaozx-cn/sglang/commit/63aa7f7ca193eb628359b651f00553853ce77f65)（`a5-k3-0828`）。以下链接对应本文三项修改及外部融合算子实现。

| 设计内容 | 对应 Commit |
| --- | --- |
| 共享专家双流：共享 MLP 放到备用流，attention-TP AG/RS 保留在主流 | [f3ca9cb559](https://github.com/zhaozx-cn/sglang/commit/f3ca9cb559d3dd34ff7ff41d2ad4147fbff4f6db) |
| FIAS V2：DSpark MLA/MHA 接入、布局适配和图内长度更新 | [8491b10adf](https://github.com/zhaozx-cn/sglang/commit/8491b10adf34d76d4bedf94530750e816945d53a) |
| SGLang SiTU＋MXFP8 Quant 包装器、分派条件与 GMM2 scale 交接 | [ff2f4caf69](https://github.com/zhaozx-cn/sglang/commit/ff2f4caf6904fae4072dcab9f1879a8c53b1125b) |
| 外部 A5 AscendC SiTU＋MXFP8 融合算子、注册、构建与测试 | [bc34b34725](https://github.com/zhaozx-cn/sgl-kernel-npu/commit/bc34b347259e062ed43a9edc7abf656444d93339) |
